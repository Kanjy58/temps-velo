L.Permalink={getMapLocation:function(layers){'use strict';var zoom,center;var layers=(layers)?layers:[];if(window.location.hash!==''){var hash=window.location.hash.replace('#','');var parts=hash.split(',');if(parts.length>=3){center={lat:parseFloat(parts[0]),lng:parseFloat(parts[1])};zoom=parseInt(parts[2].slice(0,-1),10);if((parts.length>=4)&&(parts[3]!=="")){layers=parts.slice(3);}}
return{zoom:zoom,center:center,layers:new Set(layers)};}
return{layers:new Set(layers)};},setup:function(map,layers,updateLayers){'use strict';var shouldUpdate=true;var currentLayers=layers;updateLayers(currentLayers);var updatePermalink=function(){if(!shouldUpdate){shouldUpdate=true;return;}
